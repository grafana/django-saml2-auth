...
| **TRIGGER.CUSTOM\_TOKEN\_QUERY** | A method to be called to create a custom JWT for the user. This method will be called between `BEFORE_LOGIN` and `AFTER_LOGIN` triggers and should return the query params containing the token (e.g., `?token=jwt.token.encripted`). This method should accept ONE parameter of user.                                                                                                                                                                                                                                                                                             | `str`            | `None`                                                                                                                                   | `my_app.models.users.get_custom_token_query`         |
...
### Custom token query trigger

This is an example of a function that could be passed to the `TRIGGER.CUSTOM_TOKEN_QUERY` (it uses the [DRF Simple JWT library](https://github.com/jazzband/djangorestframework-simplejwt/blob/master/docs/index.rst)):

``` python
from rest_framework_simplejwt.tokens import RefreshToken

def get_custom_token_query(user):
    """Create token for user and return url query"""
    refresh = RefreshToken.for_user(user)
    query = "?%s%s%s%s%s" % ("refresh=", refresh, "&", "access=", str(refresh.access_token))
    return query
```
