...
| **TRIGGER.CUSTOM\_TOKEN\_QUERY** | A hook function to create a custom JWT for the user. This method will be called between `BEFORE_LOGIN` and `AFTER_LOGIN` triggers and should return the query params containing the token (e.g., `?token=encoded.jwt.token`). This method accepts one parameter: `user`.                                                                                                                                                                                                                                                                                             | `str`            | `None`                                                                                                                                   | `my_app.models.users.get_custom_token_query`         |
